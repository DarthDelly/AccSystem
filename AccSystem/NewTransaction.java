...
public class NewTransaction {
    ...
    public NewTransaction() {
        ...
        NewTransactionCard ntCard = new NewTransactionCard();
        TransactionsCard tCard = new TransactionsCard();
        AccountsCard aCard = new AccountsCard();
        GeneralJournalCard jCard = new GeneralJournalCard();
        GeneralLedgerCard gCard = new GeneralLedgerCard();
        BalanceSheetCard bCard = new BalanceSheetCard();

        addCard("newTransaction", ntCard);
        addCard("transactions", tCard);
        addCard("accounts", aCard);
        addCard("generalJournal", jCard);
        addCard("generalLedger", gCard);
        addCard("balanceSheet", bCard);

        cardLayout.show(contentPanel, "newTransaction");
        bgPanel.add(contentPanel);

        JButton loadBtn = new JButton("Load CSV");
        loadBtn.setBounds(650, 20, 120, 30);
        styleMainButton(loadBtn);
        loadBtn.addActionListener(e -> {
            ...
            for (int i = 0; i < loadedModel.getRowCount(); i++) {
                ...
                for (int j = 0; j < 5; j++) {
                    Object val = loadedModel.getValueAt(i, j);
        ...
        styleMainButton(saveBtn);
        saveBtn.addActionListener(e -> JOptionPane.showMessageDialog(frame, "Save action triggered (implement actual save)."));
        bgPanel.add(saveBtn);
        ...
        newTransBtn.addActionListener(e -> showCard("newTransaction"));
        ...
        transBtn.addActionListener(e -> {
            Component c = getCard("transactions");
            if (c instanceof TransactionsCard) ((TransactionsCard) c).refreshFromJournal();
            showCard("transactions");
        });
        ...
        accountsBtn.addActionListener(e -> showCard("accounts"));
        ...
        journalBtn.addActionListener(e -> {
            Component comp = getCard("generalJournal");
            if (comp instanceof GeneralJournalCard) ((GeneralJournalCard) comp).refreshTable();
            showCard("generalJournal");
        });
        ...
        ledgerBtn.addActionListener(e -> showCard("generalLedger"));
        ...
        balanceBtn.addActionListener(e -> showCard("balanceSheet"));
    ...
    private void addCard(String name, Component comp) {
        contentPanel.add(comp, name);
        cardMap.put(name, comp);
    }

    private Component getCard(String name) {
    ...
    private class NewTransactionCard extends JPanel {
        ...
        public NewTransactionCard() {
            ...
            addBtn.addActionListener(e -> {
                ...
                journalEntries.add(new JournalEntry(date, description, debit, credit, amount));
    ...
    private class TransactionsCard extends JPanel {
        ...
        public void refreshFromJournal() {
            ...
            for (JournalEntry e : journalEntries) {
                model.addRow(new Object[]{e.date, e.description, e.debitAccount, e.creditAccount});
            }
        }
    }
    ...
    private class AccountsCard extends JPanel {
        public AccountsCard() {
            ...
            model.addRow(new Object[]{"Cash – (ASSETS)", "ASSETS", "0.00"});
            model.addRow(new Object[]{"Accounts Payable – (LIABILITIES)", "LIABILITIES", "0.00"});
        }
    }
    ...
    private static class JournalEntry {
        ...
        JournalEntry(String date, String description, String debitAccount, String creditAccount, String amount) {
            this.date = date;
    ...
    public static class LOGIN {
        public static String loggedInUser = "defaultUser";

        public LOGIN() {
         
            JOptionPane.showMessageDialog(null, "LOGIN.stub invoked (replace with real LOGIN).");
        }
    }
    ...
    public static class TransactionFolder {
        ...
        public static void addTransaction(String date, String description, String debit, String credit, String amount, String accountName) {
            ...
            System.out.println("Stub addTransaction: " + date + " | " + description + " | " + debit + " | " + credit + " | " + amount + " | " + accountName);
        }
